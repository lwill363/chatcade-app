...
const schema = z.object({
  username: z
    ...
    .optional()
    .or(z.literal("")),
  displayName: z.string().max(100, "At most 100 characters").optional().or(z.literal("")),
  bio: z.string().max(500, "At most 500 characters").optional().or(z.literal("")),
});
...
export function UserSettingsModal({ isOpen, onClose }: UserSettingsModalProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    const payload: { username?: string; displayName?: string | null; bio?: string | null } = {};
  ...
  const handleLogout = async () => {
    ...
    if (refreshToken) {
      await logoutApi({ refreshToken }).unwrap().catch(() => {});
    }
    clearStoredRefreshToken();
    dispatch(logout());
            ...
            <div className="rounded-xl border border-white/8">
              <div className="h-16 rounded-t-xl relative bg-raised">
                <div className="absolute -bottom-8 left-1/2 -translate-x-1/2">
                  <Avatar username={avatarName} size="xl" className="ring-4 ring-surface" />
                </div>
              </div>
              <div className="bg-inset rounded-b-xl px-4 pt-10 pb-4 text-center">
                <p className="text-foreground font-bold text-base leading-tight">{displayLabel}</p>
